...
const rawData = fs.readFileSync("C:\\Users\\HP\\Desktop\\getData\\PROJET04\\server\\AuthorsPublications\\output_dblp.json");
...
async function insertData() {
     const author = jsonData.authors[0];
    let id;
         const [rest]= await db
        .promise()
        .query( 'SELECT chercheur_id FROM chercheur WHERE nom_complet = ?', [
          author.name,
        ]);
        if (rest.length > 0) {
          id = rest[0].chercheur_id;
         await db
          .promise()
          .query(
            `UPDATE chercheur SET normaliz = ? WHERE nom_complet = ?`,
            [normalizeName(author.name), author.name]
          );
      }
      for (const pub of author.publications) {
        ...
        for (const cher of pub.authors) {
          ...
          if (normalizeName(cher) === normalizeName(author.name) ){
            
           
            continue; // Skip if the author is the same as the current one
          }
