...
const rawData = fs.readFileSync("final_merged_authors_with_venue_merge_2.json");
...
//     // const subjectAreas = pub.venue_details?.scimago?.subject_area_and_category || ["unknown"];
...
async function insertData() {
  for (const author of jsonData.authors) {
    const element = await checkexistname_lmcs(author.name);
    if (element !== null) {
      let chercheurId;
      let cherid;
      const [row1] = await db
      ...
      if (row1.length > 0) {
        chercheurId = row1[0].chercheur_id;
      } else {
      ...
      // console.log(`le cher ${element} est inseré a la position ${chercheurId}`)
      let cpt = 0;
      let cpt1 = 0;
      for (const pub of author.publications) {
        ...
        let confId;
        let pubid;
        // if (venueDetails?.scimago?.title === undefined) {
        //   let tit = pub.venue;
        // } else {
        //   let tit = venueDetails.scimago.title;
        // }
        ...
        if (confRow.length > 0) {
          ...
          //console.log(subjectAreaString);
          
          // console.log(" name : "+tit)
          // console.log(" venue: "+pub.venue)
          // console.log("scima : "+pub.venue_details?.scimago?.title)
          // console.log(" confid "+confId);
          const [resC] = await db
          .promise()
          .query(
            `INSERT IGNORE INTO conf_journal (conf_id, nom, type, thematique, scope, lieu, periode) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [
              confId,
              tit,
              venueDetails?.publication_type || null,
              subjectAreaString || null,
        ...
        const [pubRow] = await db
          ...
          console.log("pubRow", pubRow.length);
        if (pubRow.length > 0) {
          ...
          // console.log(`le pub ${pubid} est inseré`);
        }
...
// checkexistname_lmcs("m koudil ").then(result => {
//     if (result === null) {
//         result = "Non trouvé";
//     }
//     console.log(`Nom original: "m koudil", Nom trouvé: ${result}`);
// });
// let i = 0;
// for (const author of jsonData.authors) {
//     const wordsArray = normalizeName(author.name)
//        for( const pub of author.publications ){
//           for( const cher of pub.authors){
//             i++;
//             const ele = normalizeName(cher);
//             const n = ele.split(/\s+/)
//             const nomTrouve = lmcs.find(nom => nom.includes(n[0]));
//             if (nomTrouve) {
//                 console.log(wordsArray+" : "+cher+" -> "+n[0]+" -> "+nomTrouve)
//             } else {
//                 console.log(wordsArray+" : "+cher+" -> "+ele)
//             }
//           }
//        }
// }
...
//     for (const cher of publication.authors) {
//         const element = cher;
//         checkexistname_lmcs(element).then(result => {
//             if (result === null) {
//                 result = "Non trouvé";
//             }
//             console.log(`Nom original: ${element},  --------> ${result}`);
//         });
//     }

// }
// const element = author.name;
